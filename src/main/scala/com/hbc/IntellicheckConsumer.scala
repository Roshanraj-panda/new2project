...
case class IntellicheckConsumerClass(val servers:String,
                                topic: String,
                                groupid:String,
                                serializer:String = classOf[StringDeserializer].getCanonicalName,
                                deserializer:String = classOf[KafkaAvroDeserializer].getCanonicalName,
                                autoOffset:String="earliest",
                                schemaRegestry:String) extends LoggerClass {
  ...
  def consume(props: Properties, spark: SparkSession, jprops: Properties, env:String) = {
    info("Initializing consume class with condumer properties")
   ...
   * Provides a service as described.
   *
   * This method takes the json formatted string(header string) and
   ...
   * Provides a service as described.
   *
   * This method takes the json formatted string(detail records string) and
   * convert it into dataframe.
   */
  ...
  def dfilter(jsonstring:String, columns:Seq[String], spark: SparkSession):sql.DataFrame = {
    ...
    val trancolumn = if(jsonstring.contains("hbc_sub_trantype")){
      df.select("hbc_sub_trantype").na.drop().collectAsList().get(0).getLong(0)
    }else{""}
    val df3=df2.withColumn("hbc_sub_trantype", lit(trancolumn)).withColumn("hbc_payment_svce_swipecode", lit(swipecode)).withColumn("joinc", lit(1))
    df3
  }
...
object IntellicheckConsumer extends LoggerClass {
